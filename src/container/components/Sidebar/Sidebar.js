...
import IconExpansionTreeView from '../TreeViewUI/RichTreeView';
...
export default function Sidebar({ sidebarToggle, toggleTheme }) {
    ...
    const findParent = (node) => {
      for (const _node of node) {
        if (_node.id === parentId) {
          console.log('_node.id === parentId: ', _node.id === parentId);
          return _node;
        }
        if (!_node.children) {
  ...
  const getExplorerTreeData = (id) => {
    requestApi('get', `/api/explorers/tree?treeID=${id}`, {})
      .then((res) => {
        dispatch(setCurrentTreeData([res.data])); // redux에 저장
      })
      .catch((error) => {
        console.log(`/api/explorers/tree?treeID=${id}`, error);
      });
  ...
  // getCurrentTreeData 초기값 지정
  useEffect(() => {
    requestApi('get', `/api/explorers/tree`, {})
      .then((res) => {
        dispatch(setCurrentTreeData(res.data.children)); // redux에 저장
      })
      .catch((error) => {
        console.log(`/api/explorers/tree`, error);
      });
  }, []);
  //
  const currentTreeData = useSelector(
    (state) => state.explorer.currentTreeData
  );
  const selectedId = useSelector((state) => state.explorer.selectedTreeId);

  // selectedId redux에 저장
  const handleClickItem = (id) => {
    dispatch(setSelectedTreeId(id));
    handleChildrenData(id);
  };
  // children data 가져오기
  const childrenData = useSelector((state) => state.explorer.treeChildrenData);
  // 아이템 클릭 시 해당 아이템의 children 데이터 요청하기
  const handleChildrenData = (id) => {
    requestApi('get', `/api/explorers/tree?treeID=${id}`, {})
      .then((res) => {
        dispatch(setAddChildrenTreeData(res.data));
      })
      .catch((error) => {
        console.log(`/api/explorers/tree?treeID=${id}`, error);
      });
  ...
  useEffect(() => {
    ...
    // console.log('redux에 저장된 getCurrentTreeData :: ', currentTreeData);
  }, [childrenData]);
    ...
    <div className="sidebar">
      <div className="sidebarWrap">
        ...
        <div className="sidebarContents">
          ...
          {/* <Tree9999
            getNodeId={getSelectedId}
            children={addFakeChildren}
            data={fakeData}
            addChildrenItemsToParent={addChildrenItemsToParent}
          /> */}
        </div>
        {/* <div>
